 ...
 * chaque requête HTTP, n'exige aucun attribut Secure/SameSite/consentement
 * cookie, et suffit largement à survivre à la navigation interne (même
 * origine). Aucune IP stockée, aucun fingerprinting, aucun historique de
...
export function getAttributionForSubmit(): AttributionPayload {
  const fallback: AttributionPayload = {
    ...
    conversionPage: typeof window !== "undefined" ? window.location.pathname.slice(0, 500) : null,
    referrer: null,
    utmSource: null,
    utmMedium: null,
    utmCampaign: null,

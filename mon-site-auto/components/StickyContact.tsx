...
export default function StickyContact() {
  ...
  useEffect(() => {
    function onMobileMenu(event: Event) {
      const { open } = (event as CustomEvent<{ open?: boolean }>).detail ?? {};
      setMobileMenuOpen(Boolean(open));
    }

    window.addEventListener(MOBILE_MENU_EVENT, onMobileMenu);
    return () => window.removeEventListener(MOBILE_MENU_EVENT, onMobileMenu);
  }, []);

  return (
    <div
      className={`fixed bottom-5 right-5 z-50 flex flex-col items-end gap-2.5 transition-all duration-300 md:bottom-6 md:right-6 md:gap-3 ${
...
function WhatsAppIcon() {
  return (
    <svg
      ...
      width="26"
      height="26"
      className="h-[21px] w-[21px] md:h-[26px] md:w-[26px]"
      viewBox="0 0 24 24"
      fill="currentColor"
    >

...
describe("Autologgia -> Synergy", () => {
  ...
  it("forwards vehicle and history context without trusting a tenant field", async () => {
    const response = await contact(request("/api/contact", {
      ...
      vehicleSlug: "vehicle-test", vehicleName: "Véhicule test", historyVehicleSlug: "vehicle-test", attribution,
    }));
    expect(response.status).toBe(200);
    expect(forwardedPayload()).toMatchObject({ formKey: "vehicle_history", product: { type: "vehicle", slug: "vehicle-test", label: "Véhicule test" } });
